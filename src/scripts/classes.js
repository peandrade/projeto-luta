...
class Stage {
    ...
    renderCharacter(character, characterElement) {
        characterElement.querySelector('.name').innerHTML = `${character.name} - ${character.life.toFixed(1)} HP`;
    ...
    doAttack(attacking, attacked) {
        ...
        if (finalAttackPower > finalDefensePower) {
            ...
            this.battleLog.addMessage(`${attacking.name} causou ${finalAttackPower.toFixed(1)}`);
        } else {
